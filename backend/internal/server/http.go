...
func NewHTTPServer(qs *queue.Service, serverCfg config.ServerConfig, rateLimitStore fiber.Storage, authCfg config.AuthConfig, gatherer prometheus.Gatherer, userStore *users.Store, version string) *HTTPServer {
	...
	api.Get("/topics/:topic/consumer-groups", jwtAuth, s.requireAdmin(), s.listConsumerGroups)
	api.Post("/topics/:topic/consumer-groups", jwtAuth, s.requireAdmin(), s.registerConsumerGroup)
	api.Delete("/topics/:topic/consumer-groups/:group", jwtAuth, s.requireAdmin(), s.unregisterConsumerGroup)
