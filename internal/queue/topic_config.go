...
func (s *Service) GetTopicConfig(ctx context.Context, topic string) (*TopicConfig, error) {
	...
	err := s.pool.QueryRow(ctx,
		`SELECT topic, max_retries, message_ttl_seconds, max_depth FROM topic_config WHERE topic = $1`,
		topic,
	).Scan(&cfg.Topic, &cfg.MaxRetries, &cfg.MessageTTLSeconds, &cfg.MaxDepth)
...
func (s *Service) UpsertTopicConfig(ctx context.Context, cfg TopicConfig) error {
	_, err := s.pool.Exec(ctx, `
		INSERT INTO topic_config (topic, max_retries, message_ttl_seconds, max_depth, updated_at)
		VALUES ($1, $2, $3, $4, now())
		...
		SET max_retries         = EXCLUDED.max_retries,
		    ...
		    updated_at          = now()
	`, cfg.Topic, cfg.MaxRetries, cfg.MessageTTLSeconds, cfg.MaxDepth)
	if err != nil {
...
func (s *Service) ListTopicConfigs(ctx context.Context) ([]TopicConfig, error) {
	rows, err := s.pool.Query(ctx,
		`SELECT topic, max_retries, message_ttl_seconds, max_depth FROM topic_config ORDER BY topic ASC`)
	if err != nil {
	...
	for rows.Next() {
		var cfg TopicConfig
		if err := rows.Scan(&cfg.Topic, &cfg.MaxRetries, &cfg.MessageTTLSeconds, &cfg.MaxDepth); err != nil {
